...
public static function logConfigChange($config, $oldValues, $newValues, $metadata = [])
{
    // Safely convert arrays to comparable format
    $oldValuesFlat = [];
    $newValuesFlat = [];
    
    foreach ($oldValues as $key => $value) {
        $oldValuesFlat[$key] = is_array($value) ? json_encode($value) : $value;
    }
    
    foreach ($newValues as $key => $value) {
        $newValuesFlat[$key] = is_array($value) ? json_encode($value) : $value;
    }
    
    // Find changed fields
    $changedFields = [];
    foreach ($newValuesFlat as $key => $value) {
        if (!isset($oldValuesFlat[$key]) || $oldValuesFlat[$key] != $value) {
            $changedFields[] = $key;
        }
    }
